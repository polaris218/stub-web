...
export default class LocationMap extends Component {
  ...
  renderEntitiesToShowOnMap() {
    ...
    if ((!entities || (entities.length === 0)) && (!task || (!task.customer_exact_location && !task.additional_addresses))) {
    ...
    return (
      <LocationMapV2 task={this.props.task} theme="RH" showLocation={STATUS_META_DATA[task.status].showEntityPinOnMap} estimate={ this.props.estimate } entities={filteredData} showDirections={showDirections} customerView={customerView} height={ this.props.mapHeight } hideInfoInitially showEstimateOverlay={true} />
    );
  }
