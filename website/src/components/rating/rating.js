...
export default class Rating extends Component {
  ...
  onChangeRate(rate) {
    ...
    if (this.state.ratedOnce === -1) {
      this.setState({ rate, rateButton: true, feedback, ratedOnce: rate });
    } else {
  ...
  sendClick() {
    ...
    sendRecommendation(company_name, task_url, customer_id, customer_name, email_addresses, email_content, team_members).then((res) => {
      ...
      this.setState({ emailConfirmationText: errorMsg, loading: false, errorOccurred: true, recommendForm: false, emailError: false, });
    });
  }
  ...
  renderForm() {
    let negativeRateLimit = null;
    let positiveRateLimit = null;
    if (this.props.company.rating_type === null || typeof this.props.company.rating_type === 'undefined') {
        negativeRateLimit = 3;
        positiveRateLimit = 4;
    }
      ...
      positiveRateLimit = 7;
        ...
        negativeRateLimit = 0;
        positiveRateLimit = 1;
    } else if (this.props.company.rating_type === 'FiveStar') {
        negativeRateLimit = 3;
        positiveRateLimit = 4;
    }
    ...
    //   min_rating = this.props.company.min_acceptable_rating;
    // }
    if (this.props.social_links && typeof this.state !== 'undefined' && this.state.ratedOnce >= min_rating) {
      displaySocialsLinks = this.props.social_links['facebook'] ||
        this.props.social_links['yelp'] ||
        this.props.social_links['angieslist'] ||
        this.props.social_links['google'];
