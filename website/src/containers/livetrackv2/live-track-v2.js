...
import styles from './live-track-v2.module.scss';
...
import { LiveTrackMainV2 }  from '../../components/live_index';
import config from '../../config/config'
import { parseQueryParams } from '../../helpers';
const env = config(self).env;

export default class LiveTrackV2 extends Component {
  ...
  setProps(props) {
    ...
    } else if (props.match.params && props.match.params.company_name && props.match.params.task_url){
      company_name = props.match.params.company_name;
      task_url = props.match.params.task_url;
    }
    ...
    if (typeof this.props.useSandboxUrl !== 'undefined') {
      ...
      getLiveTask(this.state.company_name, this.state.task_url).then((res) => {
        ...
        if (updateSubscriptionFromUrl && this.state.updateSubscription && subscribe !== (parsedRes.task.notifications && !parsedRes.task.notifications.email_notification_force_unsubscribed)) {
          updateTaskSubscription(this.state.company_name, this.state.task_url, subscribe).then(() => {
            this.setState({
              showAlert: true,
