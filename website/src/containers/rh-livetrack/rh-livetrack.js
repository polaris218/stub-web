...
import styles from './rh-livetrack.module.scss';
import { RHLiveTrackMain }  from '../../components/live_index';
import {getLiveTask, rate, confirmTask, declineTask, setNewLiveStatus, updateTaskSubscription} from '../../actions';
...
export default class RHLiveTrack extends Component {
  ...
  fetchContent() {
    ...
    getLiveTask('rh', this.props.match.params.task_url).then((res) => {
      ...
      if (updateSubscriptionFromUrl && this.state.updateSubscription && subscribe !== (parsedRes.task.notifications && !parsedRes.task.notifications.email_notification_force_unsubscribed)) {
        updateTaskSubscription('Restoration Hardware', this.props.match.params.task_url, subscribe).then(() => {
          this.setState({
            showAlert: true,
    ...
    return (
      <div className={styles.liveTrackPageContainer}>
        ...
        <div className={styles.savingSpinnerContainer}>
          <SavingSpinner title="Loading" borderStyle="none" color="#999999" fontColor="#999999"  />
        </div>
        }
        {!this.state.contentLoaded && !this.state.contentLoading &&
        ...
        {this.state.contentLoaded && <Helmet
          title={title}
          defaultTitle='Arrivy'
          meta={[
